...
function getKriteriaID($no_urut) {
	include('config.php');
	$query  = "SELECT id FROM tb_kriteria ORDER BY id";
...
function getKriteriaNama($no_urut) {
	include('config.php');
	$query  = "SELECT nama FROM tb_kriteria ORDER BY id";
	$result = mysqli_query($koneksi, $query);
...
function inputKriteriaPV ($id_kriteria,$pv) {
	...
	if (mysqli_num_rows($result)==0) {
		$query = "INSERT INTO kriteria (id_kriteria, bobot) VALUES ($id_kriteria, $pv)";
	} else {
		$query = "UPDATE kriteria SET bobot=$pv WHERE id_kriteria=$id_kriteria";
	}
...
function showTabelPerbandingan($jenis,$kriteria) {
	...
	?>
						...
						<select name="bobot<?php echo $urut?>" value="<?php echo $nilai?>" >
										<option value="0">-- Pilih Variabel --</option>	
										<option value="9">-- Sangat-Sangat Penting --</option>
